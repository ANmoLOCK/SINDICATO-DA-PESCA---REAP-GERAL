# Contrato de Licenciamento de Uso de Software — Sinapesc REAP

**Modelo de cláusulas** — Sinapesc Casa Nova (contrato vigente)

| | |
|--|--|
| **LICENCIANTE (autor e titular)** | Gabriel Lourran Da Silva Costa |
| **E-mail** | gabriel730costa@gmail.com |
| **LICENCIADO** | Sinapesc — Sindicato Dos Aquicultores E Pescadores De Casa Nova |
| **PRODUTO** | Sinapesc REAP (aplicativo desktop, site público de consulta por CPF e materiais associados) |
| **Data de início** | ___ / ___ / 2026 |

---

## Cláusula 1 — Autoria e propriedade intelectual

1.1. O software **Sinapesc REAP** foi desenvolvido por **Gabriel Lourran Da Silva Costa**, único titular dos direitos autorais e da propriedade intelectual sobre o código-fonte, executável, documentação, layouts, scripts e demais materiais do sistema, nos termos da Lei nº 9.610/1998.

1.2. Este contrato **não transfere** propriedade do software ao Licenciado. Concede apenas **licença de uso**, nos limites abaixo.

1.3. Nomes, logos e marcas **“Sinapesc”** exibidos na implantação pertencem ao Licenciado. O código-base rebrandável permanece do Licenciante.

---

## Cláusula 2 — Objeto da licença

2.1. O Licenciante concede ao Licenciado licença **não exclusiva**, **intransferível** e **revogável** para:

- usar o executável **SinapescREAP.exe** na secretaria do sindicato;
- operar o site público de **consulta individual por CPF**, configurado para o Licenciado;
- gerar QR codes e relatórios vinculados à implantação Sinapesc;
- cadastrar usuários administrativos conforme configuração acordada.

2.2. Ficam expressamente **vedados**, sem autorização escrita do Licenciante:

- copiar, clonar, revender ou ceder o software a terceiros;
- modificar o código-fonte ou contratar terceiros para tal, salvo acordo específico;
- usar a mesma licença em outro sindicato ou entidade;
- remover avisos de copyright ou licença.

---

## Cláusula 3 — Prazo: licença em ciclos de 12 meses e contrato de 6 anos

3.1. **Licença de uso (ciclos):** a licença opera em **períodos de 12 (doze) meses**, contados da data de início ou de cada renovação. Cada ciclo confere direito de uso pelo prazo de **12 meses**, renovável conforme esta cláusula.

3.2. **Vigência do contrato (Sinapesc — prazo atual):** o presente contrato entre as partes tem vigência de **6 (seis) anos**, contados da data de assinatura, independentemente dos ciclos de licença de 12 meses.

3.3. **Renovação automática dos ciclos:** durante os **6 (seis) anos** de vigência contratual, a licença **renova-se automaticamente** a cada **12 (doze) meses**, salvo:

- manifestação contrária de qualquer das partes com **30 (trinta) dias** de antecedência ao fim do ciclo; ou
- inadimplência de valores acordados para manutenção/licenciamento; ou
- descumprimento das cláusulas de uso.

3.4. **Após os 6 (seis) anos:** findo o prazo contratual, o contrato **encerra-se automaticamente**, salvo **renovação expressa** por **termo aditivo escrito** assinado por ambas as partes. Sem aditivo, cessa o direito de uso do software e do site configurado.

3.5. **Resumo prático**

| Item | Prazo |
|------|--------|
| Cada licença de uso | **12 meses** (renovável durante a vigência) |
| Contrato atual Sinapesc | **6 anos** a partir da assinatura |
| Depois de 6 anos | **Encerra**, salvo novo contrato/aditivo por escrito |
| Código-fonte | **Sempre** propriedade do Licenciante |

---

## Cláusula 4 — Dados e LGPD

4.1. Os dados dos associados (nomes, CPF, REAP) armazenados na planilha Google são de **responsabilidade do Licenciado** (controlador), nos termos da LGPD.

4.2. O Licenciante atua como **fornecedor do sistema** (operador ou encarregado técnico, conforme acordado), sem propriedade sobre os dados dos associados.

---

## Cláusula 5 — Suporte, atualizações e valores

5.1. Durante cada ciclo de **12 meses**, o Licenciante pode disponibilizar correções e atualizações de versão, conforme acordo comercial separado ou anexo.

5.2. Valores, forma de pagamento e escopo de suporte (horas, canal, prazo de resposta) constam de **anexo comercial** ou instrumento apartado, renovável a cada ciclo.

5.3. A falta de pagamento na renovação pode **suspender** o direito a novas versões e suporte, **sem** excluir a titularidade do Licenciante sobre o software.

---

## Cláusula 6 — Rescisão

6.1. O contrato pode ser rescindido:

- por acordo entre as partes;
- por descumprimento grave, após notificação e prazo de 15 dias para correção;
- ao final de um ciclo de 12 meses, com aviso de 30 dias (durante os 6 anos de vigência);
- **automaticamente** ao término dos **6 (seis) anos**, salvo termo aditivo de renovação.

6.2. Rescindido o contrato, o Licenciado **cessa o uso** do software e do site configurado. Os **dados na planilha** permanecem com o Licenciado.

6.3. A rescisão **não transfere** o código-fonte ao Licenciado.

---

## Cláusula 7 — Disposições gerais

7.1. Foro: Comarca de **Casa Nova/BA** (ou outro acordado entre as partes).

7.2. Alterações somente por **termo aditivo escrito** assinado por ambas as partes.

7.3. Documento complementar: [`LICENSE`](../LICENSE) na raiz do repositório (termos gerais de software proprietário).

---

## Assinaturas

**LICENCIANTE**

Gabriel Lourran Da Silva Costa  
E-mail: gabriel730costa@gmail.com  

Assinatura: _________________________  Data: ___ / ___ / ______

---

**LICENCIADO**

Sinapesc — Sindicato Dos Aquicultores E Pescadores De Casa Nova  
Representante: _________________________  
Cargo: _________________________  

Assinatura: _________________________  Data: ___ / ___ / ______
